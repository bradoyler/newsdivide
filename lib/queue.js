...
const capture = require('./captureAsync');
const persist = require('./persist');
const saveBufferToS3 = persist.saveBufferToS3;

module.exports = async function (pages, options) {
  ...
  pages.forEach(async function (page, idx) {
    ...
    q.add(() => capture.page(page, options, browser))
    ...
    .then(res => {
      browser.close();
      console.log('>>> copied:', latestCopyParams.CopySource, 'page:', idx + 1, 'of', pages.length);
      if (idx === pages.length - 1) {
        console.log('>>> Done interation');
      }
    })
    .catch(console.error);
  });
